/* eslint-disable */
const strategy = 'social'

export default function ({ app }) {

  const { $axios, $auth } = app
  console.log($auth.strategies[strategy])
  if (!$auth.loggedIn || !$auth.strategies[strategy])
    return

  const { options } = $auth.strategies[strategy]
...
function decodeToken (str) {
  ...
  str = decodeURIComponent(escape(Buffer.from(str, 'base64').toString()));
